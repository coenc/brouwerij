        ...
        <div class="col-md-10 col-md-offset-1">
            ...
            <table id="brouwtable" class="table table-responsive table-striped table-hover">
                ...
                <tbody id="brouwselbody">
                    @foreach($brouwsels as $brouwsel)
                        <tr id="brouwselid{{$brouwsel->id}}">
                            <td data-order="{{strtotime($brouwsel->datum)}}">{{date('d-m-Y', strtotime($brouwsel->datum))}}</td>
                            <td class="text-right">{{ $brouwsel->liters}}</td>
                            <td>{{ $brouwsel->beersort->code}} {{ $brouwsel->beersort->omschrijving}}</td>
                            <td title="{{$brouwsel->opmerking}}" class="hidden-xs hidden-sm">{{ str_limit($brouwsel->opmerking, $limit = 30, $end = '...') }}</td>
                            <td align="right">
                                <div class="input-group">
                                    <div class="input-group-btn">
                                        <button class="btn btn-warning btn-xs btn-detail open-modal but-spacing" value="{{$brouwsel->id}}"><span class="glyphicon glyphicon-edit"></span>Bewerk</button> 
                                        <button class="btn btn-danger btn-xs btn-delete delete-brouwsel but-spacing" value="{{$brouwsel->id}}"><span class="glyphicon glyphicon-remove"></span>Verwijder</button>
                                    </div>
                                </div>
                            </td>
                        </tr> 
        ...
        <div class="modal-dialog modal-md">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="row"> 
                    {!! Form::model($brouwsels, ['data-parsley-validate' => '', 'id'=>'frmBrouwsels', 'files' => false])  !!}
                        ...
                        <div class="col col-md-3 col-xs-5">
                            <div class="form-group error">                            
                                ...
                                {{ Form::text('datum', null, ["id" => "datepicker", "class" => 'form-control', 'required' => '']) }}
                            </div>
                        </div>

                        <div class="col col-md-3 col-xs-5">
                            <div class="form-group">
                                {{ Form::label('liters', 'Hoeveelheid:', 'class="control-label"') }}
                        ...
                        <div class="col col-md-6 col-xs-10">
                            <div class="form-group">
                                {{ Form::label('biersoort_id', 'Product:', 'class="control-label"') }} 
                                ...
                                {{ Form::select('biersoort_id', $beersrtn, null, ['class' => 'form-control', 'required' => '']) }}
                            </div>
                        </div>

                        <div class="col col-md-12 col-xs-10">
                            <div class="form-group">
                                {{ Form::label('opmerking', 'Opmerking:', 'class="control-label"') }}
                                {{ Form::textarea('opmerking', null,  ["class" => 'form-control',  'rows'=>'4' ,'required' => '']) }}
                            </div>
                        </div>
                    {!! Form::close() !!}
                    </div>
